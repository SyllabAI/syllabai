#!/usr/bin/env python3
"""finish-pending-v2 CI orchestrator (records repo bench/inputs/pending-v2/).

Completes the corpus-v2 (rev2) ingest embed that stalled 2026-09-20 20:00:25 UTC
on the then-serving key's daily quota: 584 EXTERNAL_QUESTIONS (question-card)
chunks across 61 documents remain un-embedded in production document_chunks.

WHY app-side transport (and not CI REST like embed-bridge-v2): the CI single-call
embedContent transport produces a DIFFERENT vector space than the production
Spring AI batched provider (max |diff| 8.9e-2, cross-space top-10 rank agreement
0.79 — evidence/bench-001/embed-bridge-v2/CORRECTION.md). Serving truth requires
the app's own POST /api/v1/teacher/content/documents/{id}/embed (idempotent,
resumable, embeds only pending chunks).

WHAT IT DOES (per round, key = remaining keys only, in KEY_ORDER):
  1. read the full Render env (values included), swap SYLLABAI_EMBEDDING_GEMINI_API_KEY
     to keys[idx] (all other vars preserved byte-for-byte), PUT back
  ...
  4. drive POST /{id}/embed for every pending doc; a doc is done when
     embedded + skipped == totalChunks (embedded=0 skipped=N is a cheap no-op
     on retry — idempotent)
  5. docs that fail or come back partial go to the next round with the next key

RAW KEY VALUES NEVER REACH THE REPORT — fingerprints only. The key that stalled
(index 2, sha256:797b77a879) is deliberately NOT in the default KEY_ORDER.
