...
- [ ] T-C04 T-C03 human content validation + batch-size decision (the next gate — no engineering milestone invented before it): a human reviewer answers "does the system preserve assessment content accurately enough to safely increase batch size?" against the review package — June representative questions/mark schemes, the October Q18 conflict, the October 1A 80-vs-120 conflict (REVIEW_REQUIRED, neither side wins), the batch audit artifact (CI workflow artifact / `download/t-c03-batch-audit-report.json`), DB counts evidence, and the content status lifecycle (SUGGESTED → teacher VALIDATED → servable; never automatic). If NO: fix parser/bridge first. If YES: propose the next controlled batch size (still bounded, still one batch at a time). Dependencies: T-C03 merged.
