...
- [ ] T-C03 ONE controlled Past-Papers batch end-to-end + human review, before any wider corpus run (the 3-pair proof is done; 40 batches stay parked until T-C03 review). Scope locked 2026-09-05: **a single bounded batch of real GLM-OCR QP/MS pairs through GLM-OCR → canonical documents → T-013 chunks → T-011 paper/question/mark-scheme model → bridge audit**; all imported content stays SUGGESTED; deterministic reruns (zero new rows); conflict preservation (never merged away); **no automatic embedding and no learner-facing serving**; batch audit report for human review before any wider run. Not a corpus-wide firehose. — **implemented on branch `codex/t-c03-batch` in BOTH repos (core PR #5 @ `1ab3e62`, issue #4; parser PR #1 @ `ba36ceb`)**: parser `GlmOcrPairCli` (committed markdown→five-file-bundle production command, 71/71 tests, CI green); core `GlmOcrBatchService` (firehose guard default 10, ONE transaction, unchanged T-C02 bridge per pair, in-run idempotency pass), `GlmOcrBatchAuditReport` (per-pair both passes + row-count evidence + five DB-verified invariants: all-content-suggested / no-implicit-embedding / not-learner-servable / conflict-preservation / deterministic-rerun), ops CLI `--syllabai.glmocr.batch-dir`, `GlmOcrBatchIT` (real Postgres, 3 real pairs, second-run zero-new-rows, bound refusal, audit artifact uploaded from CI); local 196/196 unit + verify BUILD SUCCESS; **CI run 33983936464 SUCCESS on core @ `4b820cc` — 196/196 unit + 20/20 IT incl. GlmOcrBatchIT 3/3; parser CI green @ `ba36ceb` (71/71 + conformance)**; the batch audit artifact is a downloadable CI workflow artifact (all 5 invariants PASS: statuses OK/OK/REVIEW_REQUIRED, findings 25/27/32, +6 documents/+91 chunks/+3 papers/+59 versions/+59 schemes/+173 points, idempotency 3/3 DUPLICATE zero new rows; evidence recorded in issue #4). Two CI iterations fixed test-expectation bugs, not product bugs (October MS has no printed total → assert null; V7 seed versions → delta-based row evidence). Stays "on branch" until the batch audit report is human-reviewed and the PRs merge — same policy as PR #3/PR #1. No new migration; no new HTTP endpoint; bridge untouched.
