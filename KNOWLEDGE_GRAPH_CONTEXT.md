> **Import note (2026-09-10, Session 27).** Imported verbatim from the operator's uploaded copy (`github.com/nawaf-al-hussain/FileUpload/blob/main/KNOWLEDGE_GRAPH_CONTEXT.md`) after full critical review (3,172 lines). The document's own `Date: 2026-09-08` predates some referenced facts (the 112-notes / 167-anchors corpus numbers were established 2026-09-10) — treat content as current. Verified on import: §5's minimum edge vocabulary matches the live V2 `knowledge_edges` CHECK constraint exactly; all ten §54 source documents exist in this repo. Reconciliation with the build sequence lives in `KNOWLEDGE_GRAPH_BUILD_PLAN.md` §14 (live-schema vocabulary alignment, phase mapping to §8A stages A–H, teacher-lens Cycle-2+ scoping, concept-identity policy). This document is canonical for KG semantics; the Master Spec hierarchy governs conflicts (per §54).
